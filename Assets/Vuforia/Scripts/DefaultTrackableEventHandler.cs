...
public class DefaultTrackableEventHandler : MonoBehaviour, ITrackableEventHandler
{
	...
	void Update () {
		...
		if (isDetected) {
			...
			messagePos.address = mTrackableBehaviour.TrackableName;	// "target": (isDetected, position.x, position.y, position.z, rotation.x, rotation.y, rotation.z, rotation.w)
			...
			messagePos.values.Add(target.transform.rotation.w);
			osc.Send(messagePos);
		}
        ...
        TrackableBehaviour.Status newStatus)
    {
                 ...
                 newStatus == TrackableBehaviour.Status.NOT_FOUND)
        {
			...
			// Send OSC Message
			OscMessage messagePos = new OscMessage();	// Instantiate OSC message object
			//messagePos.address = target.name;	// "target" instance is destroyed
			messagePos.address = mTrackableBehaviour.TrackableName;
